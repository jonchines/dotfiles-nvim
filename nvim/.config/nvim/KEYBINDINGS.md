...
| Mode | Keybinding | Description |
|------|------------|-------------|
| i, s | `<C-l>` | Expand snippet or jump to next placeholder |
| i, s | `<C-h>` | Jump to previous placeholder |
| i, s | `<C-L>` (Shift) | Cycle through snippet choices |

**Snippets**: Friendly-snippets provides snippets for many languages including Python, Lua, JavaScript, LaTeX, Markdown, and more.
...
**Note:** LaTeX support uses both **VimTeX** (for compilation, viewing, navigation) and **texlab** (for LSP features like completion and diagnostics). They work together seamlessly.

### VimTeX Commands

All VimTeX commands use the **local leader** (`,`) prefix. These keybindings are only available in `.tex` files.

#### Compilation

| Mode | Keybinding | Description |
|------|------------|-------------|
| n | `,ll` | Toggle continuous compilation (compile on save) |
| n | `,lc` | Clean auxiliary files (.aux, .log, etc.) |
| n | `,lC` | Clean ALL files (including PDF) |
| n | `,lk` | Stop compilation |
| n | `,lK` | Stop all compilations |

#### Viewing & Navigation

| Mode | Keybinding | Description |
|------|------------|-------------|
| n | `,lv` | View PDF in Zathura (opens or focuses existing window) |
| n | `,lt` | Open table of contents |
| n | `,lT` | Toggle table of contents |

#### Errors & Info

| Mode | Keybinding | Description |
|------|------------|-------------|
| n | `,le` | Show compilation errors in quickfix |
| n | `,ls` | Show compilation status |
| n | `,li` | Show VimTeX info |
| n | `,lI` | Show full VimTeX info |
| n | `,la` | Show context menu |

### LSP Features (texlab)

LaTeX files (`.tex`) automatically get LSP support via texlab with the following standard keybindings:

| Mode | Keybinding | Description |
|------|------------|-------------|
| n | `K` | Show documentation for LaTeX command/environment |
| n | `gd` | Go to definition (for labels, citations) |
| n | `gR` | Find all references (for labels) |
| n | `<leader>ca` | Code actions (quick fixes) |
| n | `<leader>rn` | Rename label/reference |
| n | `[d` | Previous diagnostic |
| n | `]d` | Next diagnostic |

### Completion

Automatic completion is provided for:
- LaTeX commands (`\command`)
- Environments (`\begin{...}`)
- Citations (`\cite{...}`)
- Labels and references (`\ref{...}`, `\label{...}`)
- Packages (`\usepackage{...}`)
- Document classes (`\documentclass{...}`)

**Trigger**: Completions appear automatically or manually with `<C-Space>`

### Build & Format

| Mode | Keybinding | Description |
|------|------------|-------------|
| n, v | `<leader>mf` | Format LaTeX file with latexindent |

### VimTeX Features & Workflow

#### Typical Workflow

1. **Start editing**: Open your `.tex` file
2. **Start continuous compilation**: Press `,ll` to enable auto-compile on save
3. **View PDF**: Press `,lv` to open/focus Zathura
4. **Edit and save**: Changes automatically recompile and update the PDF
5. **Check errors**: Press `,le` if compilation fails
6. **Navigate large docs**: Press `,lt` to open table of contents
7. **Stop when done**: Press `,lk` to stop compilation

#### Forward/Inverse Search

- **Forward Search**: Automatically works - save your `.tex` file and the PDF updates
- **Inverse Search**: Click in Zathura PDF (with Ctrl+Click) to jump to corresponding line in Neovim
- VimTeX is configured with SyncTeX for bidirectional navigation

#### Advanced Features

- **Continuous Compilation**: Enabled via `,ll` - auto-compiles on save
- **Smart Quickfix**: Filters out common warnings (overfull/underfull boxes)
- **Table of Contents**: Interactive navigation with `,lt`
- **Context Menu**: Press `,la` for context-aware actions
- **Multiple Engines**: Supports pdflatex, xelatex, lualatex

### BibTeX Support

`.bib` files have treesitter syntax highlighting and standard editing features.

### LaTeX Features Summary
- **VimTeX**: Compilation, PDF viewing (Zathura), forward/inverse search, TOC navigation
- **LSP (texlab)**: Completion, hover docs, diagnostics, go-to-definition
- **Syntax Highlighting**: Treesitter-based for `.tex` and `.bib` files
- **Snippets**: 241 LaTeX snippets via LuaSnip
- **Formatting**: latexindent integration via `<leader>mf`
- **Build System**: latexmk with automatic compilation
- **PDF Viewer**: Zathura with SyncTeX support
...
### `,l` - LaTeX (VimTeX) - Local Leader
*Only available in `.tex` files*

- `ll` - Toggle continuous compilation
- `lv` - View PDF (Zathura)
- `lc` - Clean auxiliary files
- `lC` - Clean all files (including PDF)
- `lk` - Stop compilation
- `lK` - Stop all compilations
